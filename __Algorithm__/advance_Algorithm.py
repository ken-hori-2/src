...
class Algorithm_advance():
    ...
    def Advance(self, STATE_HISTORY, state, TRIGAR, OBS):
            ...
            if self.map_unexp_area or self.FIRST:
                ...
                if not self.TRIGAR:
                    if self.NODELIST[self.state.row][self.state.column] > 0.0:
                        if self.NODELIST[self.state.row][self.state.column] == 7:
                            print("🤖 GOALに到達しました。")
                            GOAL = True
        ...
        return self.total_stress, self.STATE_HISTORY, self.state, self.TRIGAR, self.OBS, self.BPLIST, self.action, self.Add_Advance, GOAL